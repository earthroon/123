(() => {
  ...
  class WebGL2Renderer extends Renderer {
    ...
    async init() {
      ...
      float mouseProximity(vec2 mUv) {
        // 마우스가 텍스트 라인에 가까울수록 1, 멀수록 0
        // (inside면 d<=0 이라 1에 붙음)
        float dmin = 1e6;
        for (int i = 0; i < 16; i++) {
          if (i >= uRectCount) break;
          vec4 r = uRects[i];
      ...
      void main() {
        ...
        // Option1: proximity gate (computed at mouse vs rects)
      ...
      void main() {
        ...
        // Density curve: keep it classy
        float a = pow(clamp(ink.a, 0.0, 1.0), 1.35);

        // Text-line mask (still required so old ink never shows outside)
        float zone = (uRectCount > 0) ? inZones(vUv) : 0.0;
        a *= zone;
  ...
  async function boot() {
    ...
    window.addEventListener("pointermove", (e) => {
      onPointerMove(e);
      // 근접도 기반이라 포인터 움직이면 rect 갱신이 가끔 필요(특히 스크롤/리렌더 직후)
      requestRectsRefresh();
    }, { passive: true });
